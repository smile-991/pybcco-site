export const onRequest = async ({ request, env }: any) => {
  ...
  try {
    ...
    if (!clientToken) return new Response(JSON.stringify({ error: "Unauthorized" }), { status: 401, headers: corsHeaders })
    ...
    const ins = await fetch(`${env.SUPABASE_URL}/rest/v1/project_comments`, {
      ...
      body: JSON.stringify({ project_id: projectId, sender_role: "client", message }),
    })
